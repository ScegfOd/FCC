async function get_name() {
    ...
    if (name == "none"){window.location.replace('http://localhost:9000/ffc.html') }
    else {
        for (let key in name) {
...
async function get_stats() {
 //   let url = 'http://localhost:9000/stats'  there isn't an endpoint yet but is a function in service for this
    let response = await fetch(url)
    let stats = await response.json()
    if (stats == "none"){window.location.replace('http://localhost:9000/ffc.html') }
    else {
        ...
        for (let key in stats) {
            ...
            // below needs to be refactored to meet with code in java backend
            row.insertCell(0).innerHTML= stats[key].
            row.insertCell(1).innerHTML= stats[key].
            row.insertCell(2).innerHTML= stats[key].
            row.insertCell(3).innerHTML= stats[key].
            row.insertCell(4).innerHTML= stats[key].
            row.insertCell(5).innerHTML= stats[key].
        }}
}
...
async function get_mvp() {
    ...
    if (mvp == "none"){window.location.replace('http://localhost:9000/ffc.html') }
    else {
